...
export default function ContactDetail() {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        setLoading(true);
        try {
            await axios.post(`${BASE}/public/contact`, {
                fullName: name.trim(),
            ...
            try {
                await axios.post(`${BASE}/public/feedback`, {
                    fullName: name.trim(),
                    phone: '+998' + phone.replace(/\D/g, ''),
                    message: message.trim(),
                });
            } catch {
                // show success anyway — message may have been received
            }
        }
        setLoading(false);
        setSuccess(true);
    ...
    return (
        <div className="min-h-screen bg-black text-white">
            <div className="max-w-[1700px] mx-auto px-6 py-8">
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_265px] gap-6">

                    {/* Main content */}
                    <div className="flex flex-col gap-6">

                        {/* Combined card: form + contact info */}
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-8">
                                ...
                                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm text-gray-400 mb-2">Siz bilan bog&apos;lanish</label>
                                            <input
                                                type="text"
                                                value={name}
                                    ...
                                    <div>
                                        <label className="block text-sm text-gray-400 mb-2">Shikoyat</label>
                                        <textarea
                                            value={message}
                                    ...
                                    <div className="flex items-center justify-between gap-4">
                                        <label className="flex items-center gap-2.5 cursor-pointer select-none">
                                            ...
                                            <span className="text-sm text-gray-400">
                                                Foydalanish{' '}
                                                <span className="text-[#2196F3] cursor-pointer">shartlari</span>
                                                {' '}va{' '}
                                                <span className="text-[#2196F3] cursor-pointer">qoidalarini</span>
                                                {' '}qabul qilaman
                                            </span>
                                        </label>
                                        <button
                                            ...
                                            className="shrink-0 px-8 py-3 rounded-xl font-semibold text-sm bg-[#2196F3] text-white hover:bg-[#1976D2] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                                        >
                        ...
                        {/* Map */}
                        <div className="rounded-2xl overflow-hidden border border-[#1A2030]" style={{ height: '320px' }}>
                            <iframe
            ...
            {success && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
                    <div className="relative w-full max-w-sm bg-[#1C1C1C] border border-[#2A2A2A] rounded-2xl p-8 mx-4 flex flex-col items-center text-center gap-4">
                        <button onClick={() => setSuccess(false)} className="absolute top-4 right-4 text-gray-400 hover:text-white">
