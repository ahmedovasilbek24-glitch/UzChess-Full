...
export default function ProfileSettings() {
    ...
    useEffect(() => {
        ...
        // set from localStorage immediately so UI shows something fast
        try {
            const raw = localStorage.getItem('user');
            setUser(raw ? JSON.parse(raw) : { login: JSON.parse(atob(token.split('.')[1])).login ?? '' });
        } catch { router.push('/auth/sign-in'); return; }
        setLoading(false);
        // refresh from backend in background
        api.get('/user/me')
            .catch(() => api.get('/user/profile'))
            .catch(() => api.get('/user'))
            .then(r => {
                const data = r.data?.data ?? r.data;
                if (data && (data.fullName || data.login || data.phone || data.email)) {
                    const updated: UserProfile = {
                        fullName: data.fullName ?? data.full_name,
                        avatar: data.avatar ?? data.profileImage,
                        login: data.login ?? data.phone ?? data.email,
                        phone: data.phone,
                        email: data.email,
    ...
    async function saveProfile(e: React.FormEvent) {
        ...
        setEditLoad(true);
        try {
            await api.patch('/user', { fullName: editName.trim(), birthDate: editBirth || undefined });
        } catch {
            try {
                await api.put('/user', { fullName: editName.trim(), birthDate: editBirth || undefined });
    ...
    return (
        <>
                ...
                <div className="rounded-2xl p-6 bg-[#0D1117] border border-[#1A2030]">
                    ...
                    <div className="flex gap-4">
                        ...
                        <SecCard
                            ...
                            title="Telefon raqamni yangilash"
                            sub="Telefon raqamingizni yangilang"
                            onClick={() => { setShowPhone(true); setPhoneErr(''); setPhonePwd(''); setNewPhone(''); }}
                        />
                    </div>
                </div>
            </div>
            ...
            {showPhone && (
                <Modal onClose={() => setShowPhone(false)}>
                    <ModalHead title="Telefon raqamni yangilash" onClose={() => setShowPhone(false)} />
                    <div className="px-6 py-5">
                        <form onSubmit={updatePhone} className="flex flex-col gap-4">
                            <PwdInput label="Parol" value={phonePwd} onChange={v => { setPhonePwd(v); setPhoneErr(''); }} placeholder="Parolni kiriting" autoFocus />
                            <div>
                                <label className="block text-sm mb-2 text-[#9CA3AF]">Yangi telefon raqamingizni kiriting</label>
                                <div className="flex items-stretch rounded-xl overflow-hidden bg-[#0D1117] border-2 border-[#2A3040] focus-within:border-white transition-colors">
                                    <span className="flex items-center px-4 text-sm border-r border-[#2A3040] text-gray-500 select-none">+998</span>
                                    <input type="tel" value={newPhone} onChange={e => { setNewPhone(fmtPhone(e.target.value)); setPhoneErr(''); }}
                                        placeholder="__ ___ __ __"
                                        className="flex-1 bg-transparent text-sm px-3 py-3 outline-none placeholder:text-[#4B5563] text-white" />
                                </div>
                            </div>
                            {phoneErr && <p className="text-sm text-[#EF4444]">{phoneErr}</p>}
                            <button type="submit" disabled={!phonePwd || newPhone.replace(/\D/g,'').length < 9 || phoneLoad}
                                className="w-full py-3 rounded-xl font-semibold text-sm bg-[#2196F3] text-white hover:bg-[#1976D2] disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                                {phoneLoad ? 'Yuklanmoqda...' : 'Saqlash'}
                            </button>
                        </form>
                    </div>
                </Modal>
