...
export default function BookReviews() {
    ...
    useEffect(() => {
        const getReviews = async () => {
            try {
                const res = await fetch("http://localhost:3002/public/BookReviews", { cache: "no-store" });
    ...
    async function handleReportSubmit() {
        ...
        try {
            const body: Record<string, unknown> = { target: 'book', targetId: reportId };
    ...
    if (loading) {
        return (
            <section className="mt-12">
                <h2 className="mb-6 text-3xl font-bold text-white">Kitob haqida izohlar</h2>
                <div className="rounded-xl border border-[#232428] bg-[#18191B] p-8 text-center text-gray-400">Yuklanmoqda...</div>
            </section>
        );
    }
    ...
    if (reviews.length === 0) {
        return (
            <section className="mt-12">
                <h2 className="mb-6 text-3xl font-bold text-white">Kitob haqida izohlar</h2>
                <div className="rounded-xl border border-[#232428] bg-[#18191B] p-10 text-center text-gray-400">Hozircha izohlar mavjud emas</div>
            </section>
        );
    }
                                ...
                                <div className="flex justify-between">
                                    <div className="flex gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="mt-1 flex items-center gap-2">
                                                <span className="text-xs text-gray-400">{new Date(review.date).toLocaleDateString("uz-UZ")}</span>
                                                <div className="flex">
                                    ...
                                    <div className="relative" ref={openMenuId === review.id ? menuRef : undefined}>
                                        <button onClick={() => setOpenMenuId(openMenuId === review.id ? null : review.id)}>
                                            <MoreVertical size={18} className="text-gray-400 hover:text-white transition-colors" />
                                        </button>
