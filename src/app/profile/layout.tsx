...
export default function ProfileLayout({children}: { children: React.ReactNode }) {
    ...
    const avatarContent = user?.avatar
        ? <Image src={user.avatar} alt="avatar" width={64} height={64} className="w-full h-full object-cover"
                 unoptimized/>
        : initials
            ? <span>{initials}</span>
            ...
            <div className="flex gap-5">
                <aside className="flex-shrink-0 flex flex-col gap-3 w-[268px]">
                    ...
                    <div className="rounded-2xl flex items-center gap-4 px-5 py-5 bg-[#0D1117] border border-[#1A2030]">
                        <div
                            className={`w-16 h-16 rounded-full overflow-hidden flex items-center justify-center font-bold text-xl text-white flex-shrink-0 ${user?.avatar ? '' : 'bg-blue-600'}`}>
                            {avatarContent}
                        </div>
                        <div>
